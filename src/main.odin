        ...
        case c.Invalid_Value:
          ...
          switch value_err in err_detail.reason {
            ...
            case c.Invalid_Hex_Length:
              fmt.eprintfln("Hexadecimal value must be 6 digit long (RGB, 1 byte each), not %i", value_err.length)
            case c.Invalid_Hex_Number:
              fmt.eprintfln("Not an hexadecimal number: %s", value_err.number)
            case c.Unexpected_Character:
              fmt.eprintfln("Unexpected character '%v', expected a semicolon-delimited string of numbers.", value_err.char)
            case:
